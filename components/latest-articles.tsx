        ...
        <section className="py-20 relative bg-white overflow-hidden text-slate-600">
            <div className="container mx-auto px-4 sm:px-6 max-w-7xl relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
                    ...
                    <Link
                        href={t.ctaHref}
                        className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-white border border-slate-200 text-slate-800 hover:bg-slate-50 transition-all whitespace-nowrap font-bold shadow-sm hover:scale-105 active:scale-95"
                    >
                        {t.cta}
                ...
                {latestPosts.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {latestPosts.map((post, index) => (
                            <Link key={post.slug} href={t.postHref(post.slug)} className="cursor-pointer block group h-full">
