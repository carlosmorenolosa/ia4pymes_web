...
export function BlogChatWidget({ lang = "es" }: BlogChatWidgetProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2.5 pointer-events-none">
      ...
      {showTooltip && (
        <div
          className="pointer-events-auto bg-white text-slate-800 text-[13px] font-bold py-2.5 px-4 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-2 select-none animate-fade-in"
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="pointer-events-auto w-14 h-14 rounded-full bg-[#25D366] hover:bg-[#20ba5a] text-white shadow-xl shadow-green-600/20 flex items-center justify-center transition-all duration-300 relative group hover:scale-110 active:scale-95"
